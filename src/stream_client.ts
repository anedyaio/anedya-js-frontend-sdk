...
export class AnedyaStreamClient {
  ...
  private routeValueStore(payload: Uint8Array) {
    try {
      const decoded = decode(payload);
      console.log("🔍 RAW decoded valuestore payload:", decoded); // TEMP DEBUG
      ...
      console.log("🔍 Normalized data.key:", JSON.stringify(data.key)); // TEMP DEBUG
      console.log("🔍 Active subs keys:", this.valueStoreSubs.map(s => JSON.stringify(s.key))); // TEMP DEBUG
      console.log("🔍 typeof data.key:", typeof data.key, "length:", data.key?.length); // TEMP DEBUG
      this.valueStoreSubs.forEach((s, i) => {
        console.log(`🔍 sub[${i}] typeof:`, typeof s.key, "length:", s.key?.length,
          "strict equal:", s.key === data.key,
          "active:", s.active, "paused:", s.paused,
          "char codes data:", Array.from(data.key || '').map(c => c.charCodeAt(0)),
          "char codes sub:", Array.from(s.key || '').map(c => c.charCodeAt(0))
        );
      }); // TEMP DEBUG

      console.log("🔍 globalPaused:", this.globalPaused); // TEMP DEBUG
      const matched = this.valueStoreSubs.filter((s) => s.active && !s.paused && s.key === data.key);
      console.log("🔍 matched subs count:", matched.length); // TEMP DEBUG
